...
class StockFrame():
    ...
    def create_frame(self) -> pd.DataFrame:
        ...
        # make utc to new york time
        for index in range(len(self._data)):
            #print(self._data[index]["datetime"])
            #self._data[index]['new york time'] = datetime.fromtimestamp(self._data[index]["datetime"]/1000).strftime("%Y-%m-%d %H:%M:%S")
            timestampvalue = self._data[index]["datetime"] / 1000
            # This is an EST timezone timestamp from IBK
            timestampObj = datetime.fromtimestamp(timestampvalue)
            timestampObj_est = est_tz.localize(timestampObj)
            timestampObj_utc = timestampObj_est.astimezone(utc_tz)
            timestampstring = timestampObj_est.strftime("%Y-%m-%d %H:%M:%S")
            self._data[index]['new york time'] = timestampstring
    ...
    def add_rows(self, data: Dict) -> None:
        ...
        column_names = ['open', 'close', 'high', 'low', 'volume', 'new york time']
        ...
        for quote in data:
            ...
            timestampvalue = quote['datetime'] / 1000
            timestampObj = datetime.fromtimestamp(timestampvalue)
            timestampObj_est = est_tz.localize(timestampObj)
            timestampstring = timestampObj_est.strftime("%Y-%m-%d %H:%M:%S")
            new_york_time = timestampstring
            ...
            #self._data[index]['new york time'] = datetime.fromtimestamp(quote['datetime']/1000).strftime("%Y-%m-%d %H:%M:%S")
